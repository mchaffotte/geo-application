...
public class GradleDependencyAdapter {
    ...
    private static BootstrapLogger LOGGER = BootstrapLogger.logger("org.wildfly.swarm.arquillian.adapter.gradle");
    ...
    public DeclaredDependencies parseDependencies(Configuration configuration) {
        ...
        return buildDependencies(deps);
    }

    private DeclaredDependencies buildDependencies(final String dependencies) {
        final DeclaredDependencies declaredDependencies = new DeclaredDependencies();
        try (Scanner scanner = new Scanner(dependencies)) {
            while (scanner.hasNextLine()) {
                ...
                if (line.startsWith(PREFIX1)
                    ...
                    if (line.startsWith(PROJECT)) {
                        LOGGER.warn(line + " is a sub project, do not forget to add these classes in the archive");
                    } else {
                        if (!stack.isEmpty()) {
                            stack.pop();
                        }
    ...
    private String parseCoordinate(String line) {
        ...
        if (3 == coords.length) {
            ...
            if (version.contains(VERSION_UP)) {
                return coords[0] + COLON + coords[1] + COLON + version.substring(version.indexOf(VERSION_UP) + VERSION_UP.length(), version.length());
            } else {
                return line;
            }
